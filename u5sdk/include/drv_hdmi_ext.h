...
typedef HI_S32  (*FN_HDMI_GetHdrAttr)(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_HDMI_HDR_ATTR_S *pstHdrAttr); /* sky(hdr) */
...
#endif
typedef HI_S32  (*FN_HDMI_Sleep)(PM_BASEDEV_S *pdev, HI_BOOL bSleep); // sky(stby)
...
typedef struct
{
    ...
    FN_HDMI_GetHdrAttr          pfnHdmiGetHdrAttr; /* sky(hdr) */
    FN_HDMI_SetHdrMode          pfnHdmiSetHdrMode;
    FN_HDMI_Sleep       		pfnHdmiSleep; // sky(stby)
}HDMI_EXPORT_FUNC_S;
