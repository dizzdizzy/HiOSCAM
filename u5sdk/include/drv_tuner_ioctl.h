...
typedef enum hiTUNER_QAM_TYPE_E
{
    ...
    QAM_TYPE_128,
    QAM_TYPE_256,
    QAM_TYPE_VSB,	// sky(atsc)
    QAM_TYPE_QPSK,	// sky(dvbt)
    QAM_TYPE_AUTO,	// sky(dvbt,atsc)
    QAM_TYPE_ALL	// sky(atsc)
}TUNER_QAM_TYPE_E;
#define	VSB_TYPE_AUTO 	(QAM_TYPE_VSB)
#define	IS_TUNER_QAM(x)	((x)<=QAM_TYPE_256 || (x)==QAM_TYPE_AUTO) 
#define	IS_TUNER_VSB(x)	((x)==QAM_TYPE_VSB)
...
typedef struct hiTUNER_ACC_QAM_PARAMS_S
{
    ...
    HI_BOOL bSI;              		/* reverse*/
    HI_U32  bAutoMode;   			/* sky(dvbt,atsc)*/
    ...
    HI_UNF_TUNER_SIG_TYPE_E enSigType; /* sky(j83b) */
...
// sky(atsc)
typedef struct  hiTUNER_SETATSCATTR_S
{
    HI_U32        u32Port;
    HI_UNF_TUNER_ATSC_ATTR_S    stSetAtscAttr;
}TUNER_SETATSCATTR_S, *PTR_TUNER_SETATSCATTR_S;
// sky(isdbt)
typedef struct  hiTUNER_SETISDBTATTR_S
{
    HI_U32        u32Port;
    HI_UNF_TUNER_ISDBT_ATTR_S    stSetIsdbtAttr;
}TUNER_SETISDBTATTR_S, *PTR_TUNER_SETISDBTATTR_S;
...
typedef struct  hiTUNER_BLINDSCAN_PARA_S
{
    ...
    HI_U32 u32NextFreq;        		/* sky(f) */
	HI_U32 u32FirstFreq;			///< The start scan frequency in units of 100kHz. The minimum value depends on the tuner specification.
	HI_U32 u32LastFreq;				///< The stop scan frequency in units of 100kHz. The maximum value depends on the tuner specification.
...
/* sky(dishpro) */
typedef struct  hiTUNER_DISHPRO_SWITCH_S
{
    HI_U32	u32Port;
    HI_U32 	switches;
}TUNER_DISHPRO_SWITCH_S, *PTR_TUNER_DISHPRO_SWITCH_S;
...
typedef struct tag_DemodAttr_S
{
    ...
    HI_UNF_TUNER_SIG_TYPE_E	enSigType;			// sky(dvbt)
    HI_U32 u32DemodAddr;
...
{
    HI_BOOL     bInited; 		/* sky() */
    HI_BOOL     bConnected; 	/* sky() */
    HI_BOOL     bProcEnabled; 	/* sky() */
	HI_U32  	uProcMonitorCounter; /* sky() */
    HI_UNF_TUNER_SIG_TYPE_E enSigType;
    HI_U8 enI2cChannel;
    HI_UNF_TUNER_DEV_TYPE_E   enTunerDevType;
    ...
    HI_UNF_TUNER_TER_ANTENNA_POWER_E enAntennaPower;	/* sky(dvbt) */
	...
	HI_S32  (*tuner_set_atsc_attr)(HI_U32 u32TunerPort, HI_UNF_TUNER_ATSC_ATTR_S *pstAtscTunerAttr);
	HI_S32  (*tuner_set_isdbt_attr)(HI_U32 u32TunerPort, HI_UNF_TUNER_ISDBT_ATTR_S *pstIsdbtTunerAttr);
    ...
    HI_S32  (*tuner_blindscan_stop)(HI_U32 u32TunerPort, HI_U32 stflag); // sky(bs)
    ...
    HI_S32  (*tuner_set_dishpro_switches)(HI_U32 u32TunerPort, HI_U32 switches); /* sky(dishpro) */
    HI_S32  (*tuner_set_lnb_config)(HI_U32 u32TunerPort, HI_UNF_TUNER_FE_LNB_CONFIG_S *pstLNB); // sky(bs)
...
#define TUNER_SETATSCATTR_CMD _IOWR(HI_TUNER_IOC_MAGIC, 57, TUNER_SETATSCATTR_S)
#define TUNER_SETISDBTATTR_CMD _IOWR(HI_TUNER_IOC_MAGIC, 58, TUNER_SETISDBTATTR_S)
// sky(bs)
#define TUNER_BLINDSCAN_STOP_CMD _IOWR(HI_TUNER_IOC_MAGIC, 60, TUNER_DATA_S)
#define TUNER_SET_LNBCONFIG_CMD _IOWR(HI_TUNER_IOC_MAGIC, 61, TUNER_LNBCONFIG_S)
#define TUNER_PROCMONITOR_CMD _IOWR(HI_TUNER_IOC_MAGIC, 62, TUNER_DATA_S) // sky(isdbt)
#define TUNER_SET_DISHPRO_SWITCH_CMD _IOWR(HI_TUNER_IOC_MAGIC, 63, TUNER_DISHPRO_SWITCH_S) // sky(dishpro)
...
extern HI_VOID tuner_msleep(HI_U32 u32MS); /* sky(A) */
